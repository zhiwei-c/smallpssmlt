...
void RunMarkovChain(Sampler &sampler, RadianceRecord &r1, RadianceRecord &r2)
{
    ...
    double weight1 = (accept + (sampler.largeStep ? 1.0 : 0.0)) / (ScalarContributionFunction(r.Li) / b + largeStepProb);
    double weight2 = (1 - accept) / (ScalarContributionFunction(sampler.current.Li) / b + largeStepProb);
    r1.x = r.x;
    r1.y = r.y;
    r1.Li = r.Li * weight1;
